...
from proy_sales.utils import phone_regex, valida_cedula, valida_numero_flotante_positivo, valida_numero_entero_positivo,valida_ruc
import os
from datetime import date

        
class Company(models.Model):
    dni = models.CharField(verbose_name='RUC', max_length=13, blank=True, null=True , validators=[valida_ruc])
...
class Product(models.Model):
    ...
    expiration_date = models.DateField(verbose_name='Caducidad', default=timezone.now() + datetime.timedelta(days=30))
    manufacture_date = models.DateField(verbose_name='Fecha de Elaboración', default=timezone.now)
...
class ProductPrice(models.Model):
    ...
    @property
    def get_categories(self):
        return " - ".join([c.description for c in self.category.all().order_by('description')])
...
class ProductPriceDetail(models.Model):
    ...
    issue_date = models.DateField(verbose_name='Fecha Emision',default=timezone.now)
